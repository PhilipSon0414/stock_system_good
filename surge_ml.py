...
캘리브레이션 (2026-06, score_history 546건 / 100 positives, 10%+ 기준):
  기준율: 18.3%
  Tier S: 세력≥80 & 합산≥45 → 35.2% hit rate (1.92x lift)  ★ 핵심
  Tier A: 세력≥70 & 합산≥40 → 28.5% hit rate (1.55x lift)
  Tier B: 세력≥70           → 26.5% hit rate (1.45x lift)
  Tier C: 합산40~50 구간    → 25.0% hit rate (1.36x lift)
  Tier D: 그 외             → 18.3% hit rate (기준)
  ⚠ 세력 60-69 → lift=0.62x (노이즈), 합산 70+ → lift=0.52x (연속보너스 허수)
...
# 검증 결과 (2026-06, score_history 546건 / 100 positives, 10%+ 기준):
#   기준율: 18.3%
#   세력≥80 & 합산≥45: 35.2% (1.92x) ★ 핵심
#   세력≥70 & 합산≥40: 28.5% (1.55x)
#   세력 60-69: 11.4% (0.62x) ← 기준 이하 노이즈
#   합산 70+: 9.4% (0.52x) ← 연속보너스 허수 주의
_TIERS = [
    (80, 45, 'S', 0.352, 1.92),
    (70, 40, 'A', 0.285, 1.55),
    (70,  0, 'B', 0.265, 1.45),
...
_TIER_LABELS = {
    'S': '★★★★ Tier-S (세력80+합산45+ / 예상적중 ~35%)',
    'A': '★★★  Tier-A (세력70+합산40+ / 예상적중 ~29%)',
    'B': '★★   Tier-B (세력70+ / 예상적중 ~27%)',
    'C': '★    Tier-C (합산40+ / 예상~25%)',
    'D': '      Tier-D (기본 / ~18%)',
}
